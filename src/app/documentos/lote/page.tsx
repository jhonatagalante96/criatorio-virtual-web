  ...
  return (
    <div aria-busy="true" className="badge-batch-progress" role="status">
      ...
      <div>
        ...
        <span>Conferindo as aves e preparando o arquivo temporário. Não feche esta página.</span>
      </div>
    </div>
  );
}
  ...
  return (
    <section aria-labelledby="titulo-lote-concluido" className="badge-batch-result">
      ...
      <p className="badge-batch-result-lede">
        {allFailed
          ? "Não foi possível gerar crachás para as aves selecionadas. Consulte os motivos abaixo, ajuste o que for necessário e tente novamente."
      ...
      <dl className="badge-batch-summary">
        ...
        <div><dt>Arquivo completo</dt><dd>{result.aggregatePdf ? `${result.aggregatePdf.pageCount} página${result.aggregatePdf.pageCount === 1 ? "" : "s"}` : "Não disponível"}</dd></div>
      </dl>
      {result.aggregatePdf && aggregateUrl && (
        <div className="badge-batch-download">
          <div><strong>Arquivo completo temporário</strong><span>Ele reúne somente os crachás gerados neste lote.</span></div>
          <a className="auth-primary-action" download={result.aggregatePdf.fileName} href={aggregateUrl}>Baixar arquivo completo</a>
        </div>
      )}
          ...
          {result.items.map((item) => {
            ...
            return (
              <li className={item.status === "Generated" ? "is-generated" : "is-pending"} key={item.birdId}>
                ...
                <div><strong>{bird?.name ?? "Ave não disponível"}</strong><small>{bird ? `${bird.speciesPopularName} · ${bird.ringNumber ? `Anilha ${bird.ringNumber}` : "Identificação pendente"}` : "Os dados desta ave não estão disponíveis no momento."}</small></div>
...
function BatchWizard() {
  ...
  async function generateBatch() {
    ...
    try {
      ...
      setNotice({ kind: result.status === "Generated" ? "success" : "info", text: result.status === "Generated" ? "Os crachás foram gerados. Confira o resultado de cada ave." : "Nenhuma ave pode receber um crachá neste momento." });
    } catch (error) {
    ...
    <AuthenticatedShell activeNav="documents" email={session.email} farmName={farmName}>
      <main className="document-wizard-page">
        ...
        <header className="document-wizard-header">
          <div><p className="eyebrow">Documentos · lote de crachás</p><h1>Gerar crachás em lote</h1><p>Selecione as aves, escolha uma configuração e baixe o arquivo completo.</p></div>
